...
{    
    ...
    {   $orders=Auth::user()->orders->where('status','<>','Draft')->reverse();
        ...
        return view('pages.client',['orders'=>$orders,'site'=>$site,'user'=>Auth::user(),'menu_products'=>$menu_products,'menu_categories'=>$menu_categories,'menu_collections'=>$menu_collections,'categories'=>$categories,'cart'=>$cart]);
    }
}
