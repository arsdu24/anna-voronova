...
vela.productPage = function(options) {
    ...
    if (variant) {
        ...
        if (variant.available) {
            ...
            $('.productAvailability').html('<label>' + "Availability" + ':</label> ' + "In stock");
        } else {
...
vela.ajaxFilter = function() {
    ...
    vela.cache.$body.on('click', '.filterTagFullwidthButton', function() {
        if (filterContent.hasClass('active')) {
            ...
            $('<div class="filterTagFullwidthOverlay"></div>').css('display', 'none').insertAfter(filterContent);
...
vela.quickview = function() {
    ...
    selectCallbackQuickView = function(variant, selector) {
        ...
        if (variant) {
            ...
            if (variant.compare_at_price > variant.price) {
                comparePrice.html(Shopify.formatMoney(variant.compare_at_price, window.money)).show();
            } else {
                comparePrice.hide();
            }
            if (window.swatch_enable) {
...
velatheme.SlideshowVideo = (function() {
    ...
    function init($video) {
        ...
        videos[$video.attr('id')] = {
            ...
            status: $video.data('type') === 'chrome' ? 'closed' : 'background',
    ...
    function unsetBackgroundVideo(playerId) {
        $('#' + playerId).removeAttr('style').removeClass(classes.videoBackground).addClass(classes.videoChrome);
    ...
    function setBackgroundVideo(playerId) {
        var $player = $('#' + playerId).addClass(classes.videoBackground).removeClass(classes.videoChrome);
